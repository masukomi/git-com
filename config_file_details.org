...
** Options & Agentic Systems
When you've got a list of =options=, try and avoid abbreviations for anything that isn't destined for the =title=. If you do abbreviate something, be sure to add an =agent-hint= that includes an explanation of what that option means so that an agentic system will be able to choose intelligently. For example, I wanted to abbreviate =social_media_accounts= to =sm_accounts= but I doubt that agentic systems would know that I meant =social_media_accounts= and thus it probably wouldn't choose it when it should.
